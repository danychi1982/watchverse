...
assert(app.includes('.map(s => ({ s, ep: nextEpisode(s), watchedAt: latestWatchedAt(s.id) }))'), 'Continua a guardare usa attivita non riconducibile a una visione reale');
assert(!app.includes('latestWatchedAt(s.id) || s.watchedAt || s.updatedAt || s.addedAt'), 'Continua a guardare usa ancora fallback di attivita non validi');
...
assert(app.includes('class="season-chevron"'), 'Lo stato dello chevron delle stagioni non e rappresentato');
...
assert(!css.includes('.poster-badge'), 'Il CSS del badge di stato e ancora mantenuto dopo la rimozione globale');
...
assert(app.includes('topTitles'), 'Manca la classifica dei titoli con piu tentativi');
...
console.log('Fix Sprint 1 e raccomandazioni verificati');
