...
(() => {
  ...
  async function saveProfiles(syncCloud = true, strictCloud = false) {
    ...
    try {
      await window.WatchverseCloudSync.saveProfiles(state.profiles);
    } catch (error) {
      console.warn('Watchverse cloud profile sync:', error);
      if (strictCloud) throw error;
    }
  }
  ...
  function scheduleCloudRouteRefresh(page) {
    ...
    idle(async () => {
      try {
        ...
        if (state.navigationRequestId === scheduledNavigationId && state.profileSelected && parseRoute().page === page) {
          const refresh = () => {
            state.cloudRefreshTimer = null;
            if (state.navigationRequestId !== scheduledNavigationId || !state.profileSelected || parseRoute().page !== page) return;
  ...
  function metadataErrorAnalysisHtml(analysis) {
    ...
    const topTitles = analysis.topTitles.length ? `<ol class="metadata-analysis-list">${analysis.topTitles.map(row => `<li><button type="button" class="metadata-analysis-link" data-metadata-jump data-kind="${row.kind}" data-id="${esc(row.item.id)}">${esc(row.item.title || 'Titolo senza nome')}</button><strong>${row.attempts} tentativi${row.error ? ` · ${esc(row.errorCategory || 'errore')}` : ''}</strong></li>`).join('')}</ol>` : '<p class="metadata-analysis-empty">Nessun tentativo fallito registrato.</p>';
  ...
  function castSectionHtml(item) {
    ...
    if (failed) return '<div class="metadata-detail-warning"><strong>Aggiornamento manuale consigliato.</strong> Aggiorna manualmente il titolo per riprovare il recupero del cast.</div>';
  ...
  function renderProgrammingLegacy() {
    ...
    if (state.viewCache.revision === state.dataRevision && state.viewCache.programmingMarkup) {
      setMain(state.viewCache.programmingMarkup);
      return;
    }
    ...
    if (state.viewCache.revision === state.dataRevision) state.viewCache.programmingMarkup = $('#main').innerHTML;
  }
  ...
  async function scheduleTargetedMetadataRepair() {
    ...
    const repairVersion = 'localized-match-20260809-v4';
    const targets = [...state.series.map(item => ({ kind:'series', item })), ...state.movies.map(item => ({ kind:'movie', item }))].filter(({ kind, item }) => {
      ...
      const coreIncomplete = !item.poster || isImportedPlaceholder(item.overview) || parts.coreComplete === false || !meta.provider;
  ...
  function publicMetadataSourceHtml(item) {
    const meta = item.publicMetadata;
    if (meta?.manualRetryRequired || meta?.error || meta?.failedAt) return '';
  ...
  function renderSearchLegacy() {
    ...
    const allRecommended = state.viewCache.revision === state.dataRevision && state.viewCache.searchRecommendations
      ? state.viewCache.searchRecommendations
      : profileRecommendations(18);
    if (state.viewCache.revision === state.dataRevision) state.viewCache.searchRecommendations = allRecommended;
  ...
  async function setProfilePin(profile,pin){profile.pinSalt=randomSaltB64();profile.pinHash=await hashPin(pin,profile.pinSalt);profile.updatedAt=new Date().toISOString();await saveProfiles(true,true);}
  ...
  function authErrorMessage(error) {
    ...
    if (normalized.includes('rate limit') || normalized.includes('too many requests') || normalized.includes('email rate limit')) return 'Hai superato il limite di recupero password: Supabase consente al massimo 2 email di autenticazione all’ora e richiede almeno 60 secondi tra due richieste consecutive. Attendi e riprova più tardi.';
    if (normalized.includes('otp_expired') || normalized.includes('expired') || normalized.includes('invalid or has expired')) return 'Il link di recupero non è più valido o è scaduto. Richiedi una nuova email e apri il link più recente.';
    if (normalized.includes('invalid login credentials')) return 'Nome utente o password non corretti.';
    if (normalized.includes('email not confirmed')) return 'L’indirizzo email dell’account non è stato confermato.';
    if (normalized.includes('password should be at least') || normalized.includes('password must')) return 'La password deve avere almeno 6 caratteri.';
    return raw || 'Operazione di autenticazione non riuscita. Riprova tra poco.';
  }
  ...
  function showRecoveryPasswordScreen() {
    ...
    $('#recoveryForm').addEventListener('submit', async event => {
      ...
      try { await WatchverseAuth.updatePassword($('#recoveryPassword').value); showLoginScreen('Password aggiornata. Ora puoi accedere.'); } catch (error) { message.textContent = authErrorMessage(error); }
    });
  }
  ...
  function bindShellEvents() {
    ...
    document.addEventListener('click', event => {
      ...
      void route({ loader: true });
    });
    window.addEventListener('hashchange',()=>{if(state.profileSelected)route();});
    window.addEventListener('popstate',()=>{if(state.profileSelected)void route();});
