...
(() => {
  ...
  function loaderCopyForRoute(routeInfo) {
    ...
    const copy = {
      ...
      search: ['Apro la ricerca', 'Preparo catalogo condiviso e suggerimenti.'],
  ...
  function showMetadataStatus() {
    ...
    openModal('Stato aggiornamento fonti', `<div class="metadata-status-detail"><div class="metadata-status-big"><strong>${s.coveragePercent}%</strong><span>Copertura effettiva dei metadati</span><small>${cycleText}</small></div><div class="progress-track metadata-progress large"><div class="progress-fill" style="width:${s.coveragePercent}%"></div></div><div class="metadata-recap-grid"><div><strong>${s.totalTitles.toLocaleString('it-IT')}</strong><span>Titoli del profilo</span></div><div><strong>${state.catalogEntries.length.toLocaleString('it-IT')}</strong><span>Titoli nel catalogo condiviso</span></div><div><strong>${state.catalogNetworkAvoidedThisSession.toLocaleString('it-IT')}</strong><span>Download evitati</span></div><div><strong>${s.essentialIncomplete.toLocaleString('it-IT')}</strong><span>Titoli da verificare</span></div><div><strong>${s.failed.toLocaleString('it-IT')}</strong><span>Errori tecnici</span></div></div><div class="sync-source-groups">${groups.map(syncGroupHtml).join('')}</div><p class="notice"><strong>Dato coerente in tutta l’app:</strong> la copertura indica la quota di titoli con locandina e descrizione. Un ciclo può essere terminato anche se alcune fonti non hanno trovato un risultato; tali titoli restano elencati nel dettaglio e possono essere riprovati.</p><p class="notice">Watchverse controlla prima il catalogo condiviso. Descrizioni, locandine, cast ed episodi già acquisiti vengono riutilizzati da tutti i profili; voti, visioni, preferiti e note restano separati.</p>${s.active ? `<p class="metadata-live-line"><span class="inline-spinner" aria-hidden="true"></span>${s.running} elaborazioni attive e ${s.queued} titoli in coda. Puoi continuare a usare l’app.</p>` : ''}</div>`, `<button class="ghost" id="openSourceDetails">Vedi fonti</button><button class="ghost" id="openMetadataIssues">Dettaglio titoli</button><button class="secondary" id="retryMetadata">Riprova non riusciti</button><button class="primary" id="resumeMetadata">Aggiorna ora</button>`);
  ...
  function castSectionHtml(item) {
    ...
    return `<div class="metadata-loading" role="status"><span class="inline-spinner" aria-hidden="true"></span><div><strong>Caricamento automatico del cast</strong><p>Puoi continuare a usare Watchverse: la scheda si aggiornerà in background e il risultato verrà salvato.</p></div></div><div class="cast-strip cast-grid cast-skeleton-grid">${Array.from({length:10},()=>'<span class="cast-card skeleton-card" aria-hidden="true"></span>').join('')}</div>`;
  }
  ...
  function renderHome() {
    ...
    const listContent = state.homeTab === 'watch' ? `
      ...
      ${latestReleased.length ? railSection({ id:'homeReleasedRail', title:'Nuovi episodi da recuperare', description:'Gli episodi non visti usciti più di recente, senza duplicare le serie mostrate sopra.', items:latestReleased, kicker:'Uscito di recente' }) : ''}
      <section class="section"><div class="section-head"><div><h2>Film da vedere</h2><p>${watchFilms.length} titoli pronti nella watchlist. I film non entrano in “Continua a guardare”: sono semplicemente visti o da vedere.</p></div><a class="section-link" href="#/movies">Apri film →</a></div><div class="media-grid">${watchFilms.map(m => mediaCard(m,'movie')).join('')}</div></section>` : `
  ...
  function metadataLibraryBanner(kind, items) {
    ...
    const label = kind === 'series' ? 'TVmaze + Wikipedia IT/EN' : 'Wikipedia IT/EN + Wikidata';
  ...
  function renderPerson(id, query) {
    ...
    const biography=person.biography||'Biografia in aggiornamento dalle fonti pubbliche. Puoi continuare a usare Watchverse mentre la scheda viene completata.';
  ...
  function renderSearch() {
    ...
    setMain(`${recommendationBlock}<section class="content-card"><div class="section-head"><div><h2>Trova qualcosa da vedere</h2><p>Cerca prima nel catalogo già scaricato sul dispositivo, poi nelle fonti pubbliche.</p></div></div>
      ...
      <div id="searchNotice" class="notice" style="margin-top:14px">Il catalogo condiviso contiene ${state.catalogEntries.length.toLocaleString('it-IT')} titoli riutilizzabili da tutti i profili. Ricerca pubblica: TVmaze e Wikipedia.${tmdbReady?' Anche TMDB/JustWatch è configurato.':' TMDB resta facoltativo.'}</div>
      <div id="searchResults" class="search-results"><div class="empty-state"><div class="empty-icon">⌕</div><h3>Inizia a digitare</h3><p>I titoli già scaricati vengono mostrati subito, senza ripetere il download.</p></div></div></section>`);
    ...
    const run=debounce(async()=>{
      ...
      result.innerHTML=`${localHtml}${sharedHtml}${inlineCinemaLoaderHtml('Ricerca nelle fonti pubbliche', 'Sto controllando serie, film e persone disponibili.')}`;
      bindSearchActions();
      const api=publicMetadataApi();
      try{
  ...
  function importExportMarkup(){
    const p=state.importPreview,g=state.gdprPreview,profile=currentProfile();
  ...
  async function importGdprPlan(plan,replace=true){
    if(!plan)return;const profileId=state.profileId;
    ...
    try{
      ...
      const series=plan.series.map(src=>{const base=src.tvdbId?`series-tvtime-tvdb-${src.tvdbId}`:`series-tvtime-${slug(src.title)}`;const id=profileScoped(base,profileId);sourceToId.set(src.sourceKey,id);const normalizedStatus=src.status==='watchlist'?'plan':src.status;return{...src,status:normalizedStatus,id,profileId,posterGradient:gradient(src.title),backdropGradient:gradient(src.title+' hero'),providerGroups:{streaming:[],rent:[],buy:[]},cast:[],seasons:(src.seasons||[]).map(season=>({...season,episodes:(season.episodes||[]).map(ep=>({...ep,id:`${id}:s${season.number}:e${ep.episode}`}))}))};});
      const movies=plan.movies.map(src=>({...src,id:profileScoped(`movie-tvtime-${src.sourceUuid||slug(src.title)}`,profileId),profileId,posterGradient:gradient(src.title),backdropGradient:gradient(src.title+' hero'),providerGroups:{streaming:[],rent:[],buy:[]},cast:[]}));
      const progress=plan.progress.map(src=>{const seriesId=sourceToId.get(src.sourceSeriesKey);return{id:`${profileId}|${String(seriesId).split('|').pop()}:s${src.season}:e${src.episode}`,profileId,seriesId,season:src.season,episode:src.episode,title:src.title,runtime:src.runtime,watched:true,watchedAt:src.watchedAt,tvdbId:src.tvdbId,rewatchCount:src.rewatchCount,source:src.source};}).filter(x=>x.seriesId);
      setOperationProgress(10,`Salvataggio di ${series.length.toLocaleString('it-IT')} serie…`,'Inizio importazione serie.');await dbBulkPutBatched('series',series,100,(d,t)=>setOperationProgress(10+d/Math.max(1,t)*20,`Serie: ${d.toLocaleString('it-IT')} di ${t.toLocaleString('it-IT')}`,`${Math.round(d/Math.max(1,t)*100)}% delle serie salvato.`));
      setOperationProgress(31,`Salvataggio di ${movies.length.toLocaleString('it-IT')} film…`,'Inizio importazione film e watchlist.');await dbBulkPutBatched('movies',movies,250,(d,t)=>setOperationProgress(31+d/Math.max(1,t)*20,`Film: ${d.toLocaleString('it-IT')} di ${t.toLocaleString('it-IT')}`,`${Math.round(d/Math.max(1,t)*100)}% dei film salvato.`));
      setOperationProgress(52,`Salvataggio di ${progress.length.toLocaleString('it-IT')} episodi…`,'Questa è la fase più lunga.');await dbBulkPutBatched('progress',progress,500,(d,t)=>setOperationProgress(52+d/Math.max(1,t)*43,`Episodi: ${d.toLocaleString('it-IT')} di ${t.toLocaleString('it-IT')}`,`${Math.round(d/Math.max(1,t)*100)}% della cronologia episodi salvato.`));
      ...
      state.gdprPreview=null;await reloadData();state.metadataBackgroundStarted=false;idle(scheduleBackgroundMetadataSync);setOperationProgress(100,'Importazione completata.','La libreria è pronta.');await new Promise(resolve=>setTimeout(resolve,250));
  ...
  function activeProfilePanelHtml(tab,p,account,emojis) {
    ...
    if(tab==='data')return `<div class="settings-grid profile-spaced-grid data-source-layout"><section class="settings-card wide-card shared-catalog-card"><div class="section-head"><div><h3>Catalogo condiviso sul dispositivo</h3><p>I dati pubblici di film e serie vengono scaricati una sola volta e riutilizzati da tutti i profili.</p></div><span class="catalog-count-badge">${state.catalogEntries.length.toLocaleString('it-IT')} titoli</span></div><div class="shared-data-split"><div><strong>Dati comuni</strong><p>Descrizioni, locandine, cast, stagioni, episodi, identificativi e trailer ufficiali.</p></div><div><strong>Dati del profilo</strong><p>Stato di visione, episodi visti, voto, preferiti, note e configurazioni personali.</p></div></div><p class="notice success-notice">In questa sessione il catalogo ha evitato ${state.catalogNetworkAvoidedThisSession.toLocaleString('it-IT')} download e ha riutilizzato dati per ${state.catalogHydratedThisSession.toLocaleString('it-IT')} elementi.</p></section>
  ...
  function showSetupScreen() {
    ...
    $('#authRoot').innerHTML = `<section class="auth-shell"><div class="auth-card">${authBrand('Configura il tuo account familiare')}
      <h2>Primo accesso</h2><p>L’account contiene i profili Daniela ed Elena. Scegli una password nuova e sicura: non viene salvata in chiaro.</p>
  ...
  function showLoginScreen(message='') {
    ...
    $('#authRoot').innerHTML = `<section class="auth-shell"><div class="auth-card">${authBrand('La tua libreria privata')}
      <h2>Accedi</h2><p>Un solo account familiare, con librerie separate per Daniela ed Elena.</p>
