...
assert(app.includes("const targetHash = row.kind === 'series' ? `#/series/${encodeURIComponent(item.id)}` : `#/movie/${encodeURIComponent(item.id)}`;") && app.includes("history.replaceState(null, '', targetHash);") && app.includes('await route();'), 'L aggiunta da una card correlata non apre il dettaglio del titolo aggiunto');
